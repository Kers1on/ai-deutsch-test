...
const Prompt = ({ level, type: type2, topic, count }) => {
  const isFillBlank = type2 === "fill_blank";
  return `
...
- The field "type" must be exactly "${type2}" for every question.
...
dotenv.config();
ipcMain.handle("generate-test", async (_, settings) => {
  const { level, type: type2, topic, count } = settings;
  const prompt = Prompt({ level, type: type2, topic, count });
